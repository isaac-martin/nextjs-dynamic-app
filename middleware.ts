...
export async function middleware(request: NextRequest) {
  ...
  // This fails with error about not res.setPageProp is not a function (which looking on the return types is correct, however it does contradict what the docs say)
  ...
  if (request.nextUrl.pathname.startsWith('/alternative-both')) {
    await netlifyMiddlewareRequest.rewrite(new URL('/', request.url));
